# BIT 2203 Advanced Programming - Take-Away CAT

## Student Details
* **Name:** PATRICK MWANGI NG'ANG'A
* **Registration Number:** SCT221-0598/2023

## Repository Structure & Program Description
This repository contains the complete solutions for the BIT 2203 Advanced Programming Continuous Assessment Test (CAT):
* **src/URLParser.java** 
* **src/UniversityDatabase.java** - Manages JDBC university connections and queries[cite: 1].
* **src/ShoppingCartServlet.java** - Implements session management using `HttpSession`[cite: 1].
* **src/UserService.java & UserRepository.java** - Demonstrates Dependency Injection in Spring[cite: 1].
* **src/TemperatureConverter* & RMI files** - Distributed RMI architecture components[cite: 1].
* **src/LifecycleDemoServlet.java** - Servlet implementation showcasing lifecycle methods[cite: 1].
* **src/Book*, BookDAO, BookController & web/bookList.jsp** - Full Model-View-Controller (MVC) library management application[cite: 1].

## Compilation and Execution Instructions
1. Ensure Java JDK, Apache Tomcat, and MySQL Server are installed.
2. Place source files in the `/src` folder and views in the `/web` folder[cite: 1].
3. Configure the MySQL JDBC driver and Servlet API dependencies in your build path.
